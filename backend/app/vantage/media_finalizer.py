...
from .domain import DomainError, VantageRepository
...
class OriginalMediaService:
    ...
    def initiate(self, organization_id: str, user_id: str, **request: Any) -> dict[str, Any]:
        upload = self.repository.initiate_original_upload(
    ...
    def finalize(self, organization_id: str, upload_id: str,
        ...
        upload = self.repository.get_original_upload(organization_id, upload_id)
        try:
            result, sha256 = self._finalize_checked(organization_id, upload_id, upload, version_id)
        except DomainError as error:
            self.repository.record_original_upload_failure(organization_id, upload_id, error.code)
            raise
        return {
            "uploadId": upload_id,
    ...
    def _finalize_checked(
        ...
        result = self.repository._finalize_original_from_storage(organization_id, upload_id, {
    ...
    def signed_read(self, organization_id: str, upload_id: str) -> dict[str, Any]:
        upload = self.repository.get_original_upload(organization_id, upload_id)
