...
def create_vantage_router(
    ...
    def complete_upload(upload_id: str, payload: UploadComplete, context: Context) -> dict[str, Any]:
        ...
        try:
            upload = repository.get_original_upload(context.organization_id, upload_id)
            _require_home_read(context, upload["home_id"])
            return media_service.finalize(context.organization_id, upload_id, payload.versionId)
        except DomainError as error:
            _raise(error)
    ...
    def read_original(upload_id: str, context: Context) -> dict[str, Any]:
        ...
        try:
            upload = repository.get_original_upload(context.organization_id, upload_id)
            _require_home_read(context, upload["home_id"])
            return media_service.signed_read(context.organization_id, upload_id)
        except DomainError as error:
            _raise(error)
